    ...
    def __new__(cls, resource:str, default_timeout_ms:int=1000, auto_detect:bool=True):
        if auto_detect:
            ...
            if "Agilent" in idn_query_response:
                return GenericPSU(resource_name=resource, default_timeout_ms=default_timeout_ms)
            else:
...
class GenericPSU(object):
    ...
    def selectOutput(self, output:int=1):
        if output == 1 or output == 2:
            ...
            print("Error: Wrong output is selected!")
...
class AgilentPSU(GenericPSU):
    ...
    #     if self.instr!=None:
    #         self.instr.close()
    # def command(self, command_str, query_opc:bool=True, write_delay_ms:float=0.15):
    #     self.instr.write(command_str)
    #     sleep(write_delay_ms) # this delay is necessary to give time for an older instrument to process the write
    #     if query_opc:
    #         opc = self.instr.query_ascii_values("*OPC?")[0]
